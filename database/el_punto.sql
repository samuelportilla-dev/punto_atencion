...
-- Crear procedimiento almacenado para generar número de pedido
DELIMITER //
DROP PROCEDURE IF EXISTS generar_numero_pedido //
...
BEGIN
    ...
    INTO ultimo_numero
    FROM pedidos 
    WHERE numero_pedido LIKE CONCAT('PED-', fecha_actual, '-%');
    
    SET numero_pedido = CONCAT('PED-', fecha_actual, '-', LPAD(ultimo_numero, 3, '0'));
END //
DELIMITER ;

-- Crear trigger para actualizar total del pedido
DELIMITER //
...
-- Crear trigger para actualizar total cuando se elimina un detalle
DELIMITER //
...
-- CREATE USER 'elpunto_user'@'localhost' IDENTIFIED BY 'password_segura_123';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON el_punto_db.* TO 'elpunto_user'@'localhost';
-- FLUSH PRIVILEGES;
